...
class SDBCoastalDynamics(QgsProcessingAlgorithm):
    ...
    TARGET_ROI_MODE = "TARGET_ROI_MODE"
    ...
    ROI_PROCESSING_MODES = [
        "Clip Full Analysis to ROI (Crop Rasters & Vectors)",
        "Calculate Sub-Region Quantities Only (Preserve Full Rasters)",
    ]
    ...
    def shortHelpString(self):
        ...
        <div style="font-family: 'Segoe UI', Arial, sans-serif; line-height: 1.5; color: #2C3E50;">
            ...
            <h3 style="color: #D35400; margin-top: 15px; margin-bottom: 5px; border-bottom: 2px solid #D35400; padding-bottom: 3px;">🎯 Target ROI Processing Modes</h3>
            <ul style="font-size: 12px; margin-top: 5px; padding-left: 20px;">
                <li><b>Clip Full Analysis to ROI (Crop Rasters & Vectors):</b> Restricts the entire analysis to the boundary polygon, cropping all output rasters (MSI, Volumetric Change) and shoreline migration vectors strictly to the ROI extent.</li>
                <li><b>Calculate Sub-Region Quantities Only (Preserve Full Rasters):</b> Preserves the full, uncropped spatial extent across the entire study area for all generated rasters and maps, while isolating and computing exact accretion, erosion, and net sediment balance (m³) exclusively for the Target ROI polygon (reported separately in logs, tables, and the HTML dashboard).</li>
            </ul>
    ...
    def initAlgorithm(self, config=None):
        ...
        self.addParameter(
            QgsProcessingParameterVectorLayer(
                self.SHORELINE_ROI,
                "🎯 [1.1] Study Area / Target ROI (Boundary & Calculation Polygon)",
        ...
        self.addParameter(
            QgsProcessingParameterEnum(
                self.TARGET_ROI_MODE,
                "🎯 [1.2] Target ROI Processing Mode",
                options=self.ROI_PROCESSING_MODES,
                defaultValue=1,
            )
        )
        self.addParameter(
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        roi_mode_str = self.ROI_PROCESSING_MODES[roi_mode_idx]
        ...
        if target_roi_shp and target_roi_shp.isValid():
            ...
            append_log(f"      → Target ROI Mode: {roi_mode_str}", log_file_path, feedback)

            # Determine whether to clip full rasters/vectors to ROI or calculate sub-region volume only
            if roi_mode_idx == 0:
                # Mode 0: Clip Full Analysis to ROI (Crops output rasters & vectors to polygon)
                clip_osw_shp = target_roi_path
                append_log("      → [Mode 0] Full output rasters and vectors will be clipped to Target ROI.", log_file_path, feedback)
            else:
                # Mode 1: Calculate Sub-Region Quantities Only (Keeps full-extent rasters & calculates sub-region volume inside ROI)
                clip_osw_shp = None
                append_log("      → [Mode 1] Full-extent rasters preserved; Sub-region quantities calculated inside Target ROI.", log_file_path, feedback)
        ...
        self.reporter = TemporalReportGenerator()
        self.reporter.generate_layer_group_and_reports(
