...
class GradientBoostingAutoPredictAlgorithm(QgsProcessingAlgorithm):
    ...
    def name(self): return 'sdb_autopredict_gb'
    def displayName(self): return 'SDB Auto-Predict: Gradient Boosting'
    def group(self): return 'SDB Tools-Algorithms'
    def groupId(self): return 'sdb_tools_algorithms'
    def shortHelpString(self): return 'Fully automated: Finds best parameters for Gradient Boosting and generates all final outputs.'
    def createInstance(self): return GradientBoostingAutoPredictAlgorithm()
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        param_space = [Integer(50, 400, name='n_estimators'), Integer(3, 10, name='max_depth'), Real(0.01, 0.3, "log-uniform", name='learning_rate'), Real(0.7, 1.0, "uniform", name='subsample')]
        reg_search = GradientBoostingRegressor(random_state=42)
        ...
        if save_model:
            ...
            try:
                # هذه الخوارزمية لا تستخدم Scaler، لذا سنحفظها في قاموس مع قيمة None للمكونات الأخرى
                model_to_save = {
                    'model': final_model,
                    'scaler': None,
                    'min_bounds': None,
        ...
        report_path = os.path.join(out_folder, 'autopredict_gb_report.txt'); raster_out = os.path.join(out_folder, 'autopredict_gb_depth.tif')
        samples_csv = os.path.join(out_folder, 'autopredict_gb_samples.csv'); scatter_png = os.path.join(out_folder, 'autopredict_gb_scatter.png')

        with open(report_path, 'w', encoding='utf-8') as f:
            f.write("--- SDB Auto-Predict: Gradient Boosting - Final Report ---\n"); f.write(f"\nSearch iterations performed: {n_iterations}\n")
        ...
        plt.title('Final Gradient Boosting Performance'); plt.grid(True); plt.savefig(scatter_png, dpi=150); plt.close()
