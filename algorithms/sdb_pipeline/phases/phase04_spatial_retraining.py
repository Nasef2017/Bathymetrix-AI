...
def get_model_and_params(index, opt_idx=0):
    ...
    if index == 9:
        return 'MLP', MLPRegressor(max_iter=500), {'hidden_layer_sizes': [(100,), (100, 50)], 'activation': ['relu', 'tanh'], 'learning_rate_init': [0.001, 0.01]}
    if index == 10:
...
def run_phase04_spatial_retraining(algorithm, parameters, context, feedback):
    ...
    global_path = algorithm.parameterAsRasterLayer(parameters, algorithm.INPUT_GLOBAL_RASTER, context).source()
    feat_path = algorithm.parameterAsRasterLayer(parameters, algorithm.INPUT_ORIGINAL_FEAT, context).source()
    ...
    col_mode = 0
    append_log(f"MODULE 04 START: Refinement Phase | Opt={OPTIMIZER_LIST[opt_idx]}", log_path, feedback)

    z_pred3, z_true, coords_tr = extract_values(global_path, train_lyr, train_fld, col_mode, log_path, feedback)
    ...
    X_final, y_final, _ = extract_values(p_stack, train_lyr, train_fld, col_mode, log_path, feedback)
    ...
    for idx in sel_idx:
        name, model_inst, default_params = get_model_and_params(idx, opt_idx)
        ...
        try:
            ...
            if params and n_iter > 0:
                ...
                if current_opt_idx == 0:
                    search = RandomizedSearchCV(model_inst, params, n_iter=n_iter, cv=3, n_jobs=1)
                elif current_opt_idx == 1:
                    search = GridSearchCV(model_inst, params, cv=3, n_jobs=1)
                elif current_opt_idx == 2:
                    search = BayesSearchCV(model_inst, params, n_iter=n_iter, cv=3, n_jobs=1) if SKOPT_AVAILABLE else RandomizedSearchCV(model_inst, params, n_iter=n_iter, cv=3, n_jobs=1)
