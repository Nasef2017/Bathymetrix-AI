...
from .raster_post import clean_depth_map, remove_positive_pixels, slope_filter_depth
from .vector_ops import filter_by_depth, reproject_layer_if_needed
...
def run_master_pipeline(algorithm, parameters, context, feedback):
    ...
    input_raster = algorithm.parameterAsRasterLayer(parameters, algorithm.INPUT_RASTER, context)
    target_crs = input_raster.crs()
    final_water_mask = None

    max_depth = algorithm.parameterAsDouble(parameters, algorithm.MAX_DEPTH_THRESHOLD, context)
    shrink_dist = algorithm.parameterAsDouble(parameters, algorithm.SHRINK_EDGE_DIST, context)

    remove_positives_flag = algorithm.parameterAsBool(parameters, algorithm.REMOVE_POSITIVES, context)
    apply_slope_filter = algorithm.parameterAsBool(parameters, algorithm.ENABLE_SLOPE_FILTER, context)
    slope_threshold_val = algorithm.parameterAsDouble(parameters, algorithm.SLOPE_THRESHOLD, context)

    water_mask_poly = algorithm.parameterAsVectorLayer(parameters, algorithm.WATER_MASK_POLY, context)
    if water_mask_poly:
    ...
    final_train = filter_by_depth(final_train, field_depth, max_depth, context, feedback)

    enable_val = algorithm.parameterAsBool(parameters, algorithm.ENABLE_VALIDATION, context)
    final_test = None

    if enable_val:
        t_layer = algorithm.parameterAsVectorLayer(parameters, algorithm.INPUT_TEST, context)
        if t_layer:
            temp_test = os.path.join(out_dir, 'temp_reprojected_test.gpkg')
            final_test = reproject_layer_if_needed(t_layer, target_crs, temp_test, context, feedback)
            final_test = filter_by_depth(
                final_test,
                algorithm.parameterAsString(parameters, algorithm.FIELD_TEST_DEPTH, context),
    ...
    if algorithm.parameterAsBool(parameters, algorithm.ENABLE_ADAPTIVE, context):
        ...
        final_ad = reproject_layer_if_needed(ad_layer, target_crs, temp_adapt, context, feedback)
        field_ad_depth = algorithm.parameterAsString(parameters, algorithm.FIELD_ADAPTIVE_DEPTH, context)
        final_ad = filter_by_depth(final_ad, field_ad_depth, max_depth, context, feedback)
    ...
    if p3.get('OUTPUT_DEPTH_MAP') and os.path.exists(p3['OUTPUT_DEPTH_MAP']):
        ...
        clean_depth_map(p3['OUTPUT_DEPTH_MAP'], feat_stack, max_depth, p3_clamped, context, feedback)
    ...
    if path_refined and os.path.exists(path_refined):
        ...
        clean_depth_map(path_refined, feat_stack, max_depth, p4_clamped, context, feedback)
    ...
    if enable_val and final_test:
        ...
        processing.run(
            "sdb_tools:sdb_05_reporting",
            {
                ...
                'FIELD_VAL_DEPTH':  algorithm.parameterAsString(parameters, algorithm.FIELD_TEST_DEPTH, context),
                'OUTPUT_FOLDER':    out_dir
            },
            context=context, feedback=feedback, is_child_algorithm=True
        )
