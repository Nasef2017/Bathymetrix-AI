...
class LassoAutoPredictAlgorithm(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, config=None):
        ...
        self.addParameter(QgsProcessingParameterNumber(self.N_ITERATIONS, 'Number of Smart Search Iterations', type=QgsProcessingParameterNumber.Integer, defaultValue=20, minValue=10))
    ...
    def name(self): return 'sdb_autopredict_lasso'
    def displayName(self): return 'SDB Auto-Predict: Lasso'
    def group(self): return 'SDB Tools-Algorithms'
    def groupId(self): return 'sdb_tools_algorithms'
    def shortHelpString(self): return 'Fully automated: Finds best alpha for Lasso and generates all final outputs.'
    def createInstance(self): return LassoAutoPredictAlgorithm()
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        param_space = [Real(1e-4, 1e+1, "log-uniform", name='alpha')]
        reg_search = Lasso(random_state=42, max_iter=5000)
        ...
        search_results = gp_minimize(objective_function, param_space, n_calls=n_iterations, random_state=42)
        ...
        report_path = os.path.join(out_folder, 'autopredict_lasso_report.txt'); raster_out = os.path.join(out_folder, 'autopredict_lasso_depth.tif')
        samples_csv = os.path.join(out_folder, 'autopredict_lasso_samples.csv'); scatter_png = os.path.join(out_folder, 'autopredict_lasso_scatter.png')

        with open(report_path, 'w', encoding='utf-8') as f:
            f.write(f"--- SDB Auto-Predict: Lasso - Final Report ---\n\nBest Hyperparameters Found:\n"); 
        ...
        plt.title('Final Lasso Performance'); plt.grid(True); plt.savefig(scatter_png, dpi=150); plt.close()
