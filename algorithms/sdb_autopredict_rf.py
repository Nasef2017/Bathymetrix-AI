...
class RandomForestAutoPredictAlgorithm(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, config=None):
        ...
        # <<< تمت الإضافة: إضافة الخيار الجديد كمدخل للأداة. سيكون مخفياً عند استدعائه من الأداة الرئيسية
    ...
    def name(self): return 'sdb_autopredict_rf'
    def displayName(self): return 'SDB Auto-Predict: RandomForest'
    def group(self): return 'SDB Tools - Algorithms' 
    def groupId(self): return 'sdb_tools_algorithms' 
    def shortHelpString(self): return 'Fully automated: Finds best parameters for RandomForest and generates all final outputs in one step.'
    def createInstance(self): return RandomForestAutoPredictAlgorithm()
        
    # --- الدوال المساعدة (_read_samples_from_vector, _read_samples_from_file, _transform_point_if_needed) تبقى كما هي بدون أي تغيير ---
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        # --- قراءة المدخلات كما هي بدون أي تغيير ---
        ...
        if save_model:
            # نستخدم اسم ملف موحد لكي تجده الأداة الرئيسية بسهولة
            model_save_path = os.path.join(out_folder, 'trained_model.joblib')
            try:
                # نحفظ النموذج النهائي (final_model) مع الـ Scaler لأنه جزء مهم من المعالجة
                # سنحفظهم معًا في قاموس (dictionary)
                model_and_scaler_to_save = {
                    'model': final_model,
                    'scaler': scaler,
                    'min_bounds': min_bounds, # حفظ هذه القيم مهم للمعالجة المستقبلية
