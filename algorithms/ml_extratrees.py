...
class ExtraTreesAutoPredictAlgorithm(QgsProcessingAlgorithm):
    ...
    def name(self): return 'sdb_autopredict_extratrees'
    def displayName(self): return 'SDB Auto-Predict: Extra Trees'
    def group(self): return 'SDB Tools-Algorithms'
    def groupId(self): return 'sdb_tools_algorithms'
    def shortHelpString(self): return 'Fully automated: Finds best parameters for Extra Trees and generates all final outputs.'
    def createInstance(self): return ExtraTreesAutoPredictAlgorithm()
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        param_space = [Integer(50, 500, name='n_estimators'), Integer(10, 50, name='max_depth'), Integer(2, 10, name='min_samples_split'), Integer(1, 5, name='min_samples_leaf'), Categorical(['sqrt', 'log2', None], name='max_features')]
        reg_search = ExtraTreesRegressor(random_state=42, n_jobs=1)
        ...
        report_path = os.path.join(out_folder, 'autopredict_extratrees_report.txt'); raster_out = os.path.join(out_folder, 'autopredict_extratrees_depth.tif')
        samples_csv = os.path.join(out_folder, 'autopredict_extratrees_samples.csv'); scatter_png = os.path.join(out_folder, 'autopredict_extratrees_scatter.png')

        with open(report_path, 'w', encoding='utf-8') as f:
            f.write("--- SDB Auto-Predict: Extra Trees - Final Report ---\n"); f.write(f"\nSearch iterations performed: {n_iterations}\n")
        ...
        plt.title('Final Extra Trees Performance'); plt.grid(True); plt.savefig(scatter_png, dpi=150); plt.close()
