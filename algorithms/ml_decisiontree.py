...
class DecisionTreeAutoPredictAlgorithm(QgsProcessingAlgorithm):
    ...
    def name(self): return 'sdb_autopredict_decisiontree'
    def displayName(self): return 'SDB Auto-Predict: Decision Tree'
    def group(self): return 'SDB Tools-Algorithms'
    def groupId(self): return 'sdb_tools_algorithms'
    def shortHelpString(self): return 'Fully automated: Finds best parameters for a Decision Tree and generates all final outputs.'
    def createInstance(self): return DecisionTreeAutoPredictAlgorithm()
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        X_train_full, X_test, y_train_full, y_test, _, idx_test = train_test_split(X, y, np.arange(len(y)), test_size=0.3, random_state=42)
        ...
        param_space = [Integer(5, 50, name='max_depth'), Integer(2, 20, name='min_samples_split'), Integer(1, 10, name='min_samples_leaf'), Categorical(['sqrt', 'log2', None], name='max_features')]
        reg_search = DecisionTreeRegressor(random_state=42)
        ...
        if save_model:
            ...
            try:
                # هذه الخوارزمية لا تستخدم Scaler، لذلك سنحفظ النموذج فقط
                # ولجعل الأداة الجديدة تعمل مع هذا النوع أيضاً، سنحفظه في قاموس بنفس الشكل
                model_to_save = {
                    'model': final_model,
                    'scaler': None, # نرسل قيمة None للـ Scaler
                    'min_bounds': None,
                    'max_bounds': None
                }
                joblib.dump(model_to_save, model_save_path)
                feedback.pushInfo(f"Model successfully saved to: {model_save_path}")
            except Exception as e:
        ...
        report_path = os.path.join(out_folder, 'autopredict_dt_report.txt'); raster_out = os.path.join(out_folder, 'autopredict_dt_depth.tif')
        samples_csv = os.path.join(out_folder, 'autopredict_dt_samples.csv'); scatter_png = os.path.join(out_folder, 'autopredict_dt_scatter.png')

        with open(report_path, 'w', encoding='utf-8') as f:
            f.write("--- SDB Auto-Predict: Decision Tree - Final Report ---\n"); f.write(f"\nSearch iterations performed: {n_iterations}\n")
        ...
        plt.title('Final Decision Tree Performance'); plt.grid(True); plt.savefig(scatter_png, dpi=150); plt.close()
        ...
        if valid_mask.any(): flat_preds[valid_mask] = final_model.predict(flatX[valid_mask])
