...
class ElasticNetAutoPredictAlgorithm(QgsProcessingAlgorithm):
    ...
    def name(self): return 'sdb_autopredict_elasticnet'
    def displayName(self): return 'SDB Auto-Predict: ElasticNet/Ridge/Lasso'
    def group(self): return 'SDB Tools-Algorithms'
    def groupId(self): return 'sdb_tools_algorithms'
    def shortHelpString(self): return 'Fully automated: Finds best alpha & l1_ratio for ElasticNet and generates outputs.'
    def createInstance(self): return ElasticNetAutoPredictAlgorithm()
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        param_space = [Real(1e-4, 1e+1, "log-uniform", name='alpha'), Real(0.0, 1.0, "uniform", name='l1_ratio')]
        reg_search = ElasticNet(random_state=42, max_iter=5000)
        ...
        final_model = ElasticNet(random_state=42, max_iter=5000, **best_params)
        ...
        report_path = os.path.join(out_folder, 'autopredict_elasticnet_report.txt'); raster_out = os.path.join(out_folder, 'autopredict_elasticnet_depth.tif')
        samples_csv = os.path.join(out_folder, 'autopredict_elasticnet_samples.csv'); scatter_png = os.path.join(out_folder, 'autopredict_elasticnet_scatter.png')

        with open(report_path, 'w', encoding='utf-8') as f:
            f.write("--- SDB Auto-Predict: ElasticNet/Ridge/Lasso - Final Report ---\n"); f.write(f"\nSearch iterations performed: {n_iterations}\n")
            f.write("\n--- Best Hyperparameters Found ---\n"); 
            for key, value in best_params.items(): f.write(f"{key}: {value}\n")
            f.write("\n(Note: If l1_ratio is near 1.0, it's like Lasso. If near 0.0, it's like Ridge.)\n")
        ...
        plt.title('Final ElasticNet Performance'); plt.grid(True); plt.savefig(scatter_png, dpi=150); plt.close()
