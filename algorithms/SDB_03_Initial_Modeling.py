...
class SDBModule03(QgsProcessingAlgorithm):
    INPUT_STACK = 'INPUT_STACK'; INPUT_MASK = 'INPUT_MASK'; INPUT_POINTS = 'INPUT_POINTS'
    FIELD_DEPTH = 'FIELD_DEPTH'; FIELD_WEIGHT = 'FIELD_WEIGHT'
    ...
    MEDIAN_SIZE = 'MEDIAN_SIZE'; OUTPUT_FOLDER = 'OUTPUT_FOLDER'; LOG_FILE = 'LOG_FILE'
    
    PARAM_RF = 'PARAM_RF'; PARAM_GB = 'PARAM_GB'; PARAM_ET = 'PARAM_ET'
    PARAM_SVR = 'PARAM_SVR'; PARAM_MLP = 'PARAM_MLP'
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        self.append_log(f"MODULE 03 START: Optimizer = {self.OPTIMIZER_LIST[opt_idx]}", log_path, feedback)

        # 1. EXTRACT SAMPLES
        X, y, final_weights, coords = self.extract_samples(stack_path, points_layer, depth_fld, weight_fld, col_mode)
        ...
        # --- SAVE ACTUAL MODEL INPUT POINTS (UPDATED) ---
        actual_pts_path = os.path.join(out_dir, '3_Actual_Model_Input_Points.shp')
        # Pass X to save band values
        self.save_training_points(actual_pts_path, coords, y, final_weights, X, stack_path, points_layer.sourceCrs())
        QgsProject.instance().addMapLayer(QgsVectorLayer(actual_pts_path, "3_Actual_Model_Input_Points", "ogr"))

        # 2. RUN BENCHMARK
        results_df, best_algo_data = self.run_benchmarking(X, y, final_weights, sel_idx, n_iter, out_dir, feedback, opt_idx, log_path)
    ...
    # --- UPDATED SAVE FUNCTION (INDENTATION FIXED) ---
    def save_training_points(self, out_path, coords, depths, weights, X_data, ref_raster, crs):
        fields = QgsFields()
        ...
        if writer.hasError() != QgsVectorFileWriter.NoError: return
        ...
        with rasterio.open(ref_raster) as src:
            ...
            for i, (r, c) in enumerate(coords):
                ...
                # Attributes: Depth, Weight, Row, Col, Band1, Band2...
    ...
    def run_benchmarking(self, X, y, weights, indices, n_iter, out_dir, fb, opt_idx, log_path):
        ...
        for idx in [int(i) for i in indices]:
            ...
            if name in self.custom_params and self.custom_params[name]:
                if opt_idx == 2 and SKOPT_AVAILABLE: params = self.convert_to_bayes(self.custom_params[name])
                else: params = self.custom_params[name]
    ...
    def get_model_and_params(self, index, opt_idx=0):
        ...
        if index == 9: return 'MLP (Neural Net)', MLPRegressor(max_iter=500), {'hidden_layer_sizes': [(100,), (100, 50)], 'activation': ['relu', 'tanh'], 'learning_rate_init': [0.001, 0.01]}
