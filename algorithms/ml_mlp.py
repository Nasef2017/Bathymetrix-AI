...
from qgis.core import (
    ...
    QgsProcessingParameterFolderDestination, QgsProcessingParameterBoolean, # <<< تمت الإضافة
...
class MlpAutoPredictAlgorithm(QgsProcessingAlgorithm):
    ...
    SAVE_MODEL = 'SAVE_MODEL' # <<< تمت الإضافة: تعريف متغير جديد لخيار الحفظ
    ...
    def initAlgorithm(self, config=None):
        ...
        # <<< تمت الإضافة: إضافة الخيار الجديد كمدخل للأداة
        self.addParameter(QgsProcessingParameterBoolean(self.SAVE_MODEL, 'Save Trained Model for Re-use', defaultValue=False, optional=True))

    def name(self): return 'sdb_autopredict_mlp'
    def displayName(self): return 'SDB Auto-Predict: MLP'
    def group(self): return 'SDB Tools-Algorithms'
    def groupId(self): return 'sdb_tools_algorithms'
    def shortHelpString(self): return 'Fully automated: Finds best parameters for MLP and generates all final outputs.'
    def createInstance(self): return MlpAutoPredictAlgorithm()

    # --- الدوال المساعدة (_read_samples_from_vector, etc.) تبقى كما هي بدون تغيير ---
    ...
    def processAlgorithm(self, parameters, context, feedback):
        if not libs_installed: raise RuntimeError('Required libraries are not installed. Please run `pip install scikit-learn scikit-optimize rasterio joblib` in OSGeo4W Shell.')
        ...
        # <<< تمت الإضافة: قراءة قيمة خيار الحفظ من المدخلات
        save_model = self.parameterAsBool(parameters, self.SAVE_MODEL, context)
        start_time = time.time()
        
        # --- كل الكود التالي الخاص بمعالجة البيانات والتدريب يبقى كما هو ---
        ...
        feedback.pushInfo("\nStep 2: Starting smart search for best MLP parameters...")
        param_space = [
            Categorical(['50', '100', '50,25', '100,50', '128,64'], name='hidden_layer_sizes_str'),
            Categorical(['relu', 'tanh'], name='activation'),
            Real(1e-5, 1e-2, "log-uniform", name='alpha')
        ]
        reg_search = MLPRegressor(random_state=42, max_iter=1000, early_stopping=True, n_iter_no_change=15)
        
        @use_named_args(param_space)
        def objective_function(**params):
            if feedback.isCanceled(): return 1e9
            params['hidden_layer_sizes'] = tuple(int(u.strip()) for u in params.pop('hidden_layer_sizes_str').split(','))
        ...
        best_params['hidden_layer_sizes'] = tuple(int(u.strip()) for u in best_params.pop('hidden_layer_sizes_str').split(','))
        ...
        final_model = MLPRegressor(random_state=42, max_iter=2000, early_stopping=True, n_iter_no_change=15, **best_params)
        final_model.fit(X_train_full, y_train_full)
        
        # <<< تمت الإضافة: الجزء الخاص بحفظ النموذج المدرب
        if save_model:
            model_save_path = os.path.join(out_folder, 'trained_model.joblib')
            try:
                # هذه الخوارزمية تستخدم Scaler، لذلك سنحفظه مع النموذج
                model_and_scaler_to_save = {
                    'model': final_model,
                    'scaler': scaler,
                    'min_bounds': None, # هذه الخوارزمية لا تستخدم Clip
                    'max_bounds': None
                }
                joblib.dump(model_and_scaler_to_save, model_save_path)
                feedback.pushInfo(f"Model and Scaler successfully saved to: {model_save_path}")
            except Exception as e:
                feedback.pushWarning(f"Could not save the model. Error: {e}")

        # --- باقي الكود الخاص بالتقييم وإنشاء المخرجات يبقى كما هو ---
        ...
        report_path = os.path.join(out_folder, 'autopredict_mlp_report.txt'); raster_out = os.path.join(out_folder, 'autopredict_mlp_depth.tif')
        samples_csv = os.path.join(out_folder, 'autopredict_mlp_samples.csv'); scatter_png = os.path.join(out_folder, 'autopredict_mlp_scatter.png')

        with open(report_path, 'w', encoding='utf-8') as f:
            f.write("--- SDB Auto-Predict: MLP - Final Report ---\n"); f.write(f"\nSearch iterations performed: {n_iterations}\n")
        ...
        plt.title('Final MLP Performance'); plt.grid(True); plt.savefig(scatter_png, dpi=150); plt.close()
