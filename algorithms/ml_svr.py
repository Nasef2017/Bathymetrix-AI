...
from qgis.core import (
    ...
    QgsProcessingParameterFolderDestination, QgsProcessingParameterFileDestination, # تم التعديل هنا
...
class SVRAutoPredictAlgorithm(QgsProcessingAlgorithm):
    ...
    OUTPUT_MODEL = 'OUTPUT_MODEL' # تم التعديل هنا
    ...
    def initAlgorithm(self, config=None):
        ...
        # --- تم التغيير هنا ليتوافق مع MasterWorkflow ---
        self.addParameter(QgsProcessingParameterFileDestination(self.OUTPUT_MODEL, 'Trained Model Output (optional)', fileFilter='*.joblib', optional=True))

    def name(self): return 'sdb_autopredict_svr'
    def displayName(self): return 'SDB Auto-Predict: SVR'
    def group(self): return 'SDB Tools-Algorithms'
    def groupId(self): return 'sdb_tools_algorithms'
    def shortHelpString(self): return 'Fully automated: Finds best parameters for SVR and generates all final outputs.'
    def createInstance(self): return SVRAutoPredictAlgorithm()
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        # --- تم التغيير هنا ليتوافق مع MasterWorkflow ---
        model_output_path = self.parameterAsString(parameters, self.OUTPUT_MODEL, context)
        ...
        if vlayer: 
            ...
            # افترض أن ملف CSV يستخدم نفس نظام إحداثيات الصورة إذا لم يتم توفير طبقة فيكتور
            sample_crs = raster_crs
        ...
        feedback.pushInfo("\nStep 2: Starting smart search for best parameters (SVR can be slow)...")
        param_space = [Real(1e-1, 1e+3, "log-uniform", name='C'), Real(1e-4, 1e+1, "log-uniform", name='gamma'), Real(1e-2, 1e-0, "log-uniform", name='epsilon')]
        reg_search = SVR(kernel='rbf')
        ...
        if model_output_path:
            try:
                ...
                joblib.dump(model_and_scaler_to_save, model_output_path)
                feedback.pushInfo(f"Model and Scaler successfully saved to: {model_output_path}")
        ...
        report_path = os.path.join(out_folder, 'autopredict_svr_report.txt'); raster_out = os.path.join(out_folder, 'autopredict_svr_depth.tif')
        samples_csv = os.path.join(out_folder, 'autopredict_svr_samples.csv'); scatter_png = os.path.join(out_folder, 'autopredict_svr_scatter.png')

        with open(report_path, 'w', encoding='utf-8') as f:
            f.write("--- SDB Auto-Predict: SVR - Final Report ---\n"); f.write(f"\nSearch iterations performed: {n_iterations}\n")
        ...
        plt.title('Final SVR Performance'); plt.grid(True); plt.savefig(scatter_png, dpi=150); plt.close()
