...
def save_training_points(out_path, coords, depths, weights, X_data, ref_raster, crs):
    ...
    writer = QgsVectorFileWriter(out_path, "UTF-8", fields, QgsWkbTypes.Point, crs, "ESRI Shapefile")
...
def get_model_and_params(index, opt_idx=0):
    ...
    if index == 9:
        return 'MLP (Neural Net)', MLPRegressor(max_iter=500), {'hidden_layer_sizes': [(100,), (100, 50)], 'activation': ['relu', 'tanh'], 'learning_rate_init': [0.001, 0.01]}
    if index == 10:
...
def run_benchmarking(X, y, weights, indices, n_iter, out_dir, feedback, opt_idx, log_path, custom_params):
    ...
    for idx in [int(i) for i in indices]:
        name, base_model, default_params = get_model_and_params(idx, opt_idx)

        if name in custom_params and custom_params[name]:
            if opt_idx == 2 and SKOPT_AVAILABLE:
                params = convert_to_bayes(custom_params[name])
            else:
        ...
        try:
            model = base_model
            fit_params = {}
            if name in ['Random Forest', 'Gradient Boosting', 'Extra Trees', 'Ridge', 'Lasso', 'Decision Tree', 'SVR', 'Linear Regression']:
            ...
            if params and n_iter > 0:
                ...
                if current_opt_idx == 0:
                    search = RandomizedSearchCV(base_model, params, n_iter=n_iter, cv=3, n_jobs=1)
                elif current_opt_idx == 1:
                    search = GridSearchCV(base_model, params, cv=3, n_jobs=1)
                elif current_opt_idx == 2:
                    search = BayesSearchCV(base_model, params, n_iter=n_iter, cv=3, n_jobs=1) if SKOPT_AVAILABLE else RandomizedSearchCV(base_model, params, n_iter=n_iter, cv=3, n_jobs=1)
            ...
            save_algo_artifacts(y_val, y_p, np.abs(y_val - y_p), name, algo_dir, r2, rmse, wmape, params_str)
            results.append({'Algorithm': name, 'Model': model, 'R2': r2, 'RMSE': rmse, 'wMAPE': wmape})
            append_log(f"      > {name}: R2={r2:.3f}, RMSE={rmse:.2f}m", log_path, feedback)
    ...
    fit_params_final = {}
    if winner['Algorithm'] in ['Random Forest', 'Gradient Boosting', 'Extra Trees', 'Ridge', 'Lasso', 'Decision Tree', 'SVR', 'Linear Regression']:
        fit_params_final['sample_weight'] = weights
    final_model.fit(X, y, **fit_params_final)
    return df, {'name': winner['Algorithm'], 'model': final_model, 'score': winner['score'], 'r2': winner['R2'], 'rmse': winner['RMSE'], 'wmape': winner['wMAPE']}


def predict_map(model, stack_path, mask_path, out_path, med_size):
...
def run_phase03_initial_modeling(algorithm, parameters, context, feedback):
    ...
    custom_params = {
        'Random Forest': parse_param_string(algorithm.parameterAsString(parameters, algorithm.PARAM_RF, context)),
        'Gradient Boosting': parse_param_string(algorithm.parameterAsString(parameters, algorithm.PARAM_GB, context)),
        'Extra Trees': parse_param_string(algorithm.parameterAsString(parameters, algorithm.PARAM_ET, context)),
        'SVR': parse_param_string(algorithm.parameterAsString(parameters, algorithm.PARAM_SVR, context)),
        'MLP': parse_param_string(algorithm.parameterAsString(parameters, algorithm.PARAM_MLP, context))
    }

    stack_path = algorithm.parameterAsRasterLayer(parameters, algorithm.INPUT_STACK, context).source()
    mask_path = algorithm.parameterAsRasterLayer(parameters, algorithm.INPUT_MASK, context).source()
    points_layer = algorithm.parameterAsVectorLayer(parameters, algorithm.INPUT_POINTS, context)
    depth_fld = algorithm.parameterAsString(parameters, algorithm.FIELD_DEPTH, context)
    weight_fld = algorithm.parameterAsString(parameters, algorithm.FIELD_WEIGHT, context)
    if not weight_fld:
        weight_fld = None
    ...
    col_mode = algorithm.parameterAsInt(parameters, algorithm.COLLISION_HANDLING, context)

    append_log(f"MODULE 03 START: Optimizer = {OPTIMIZER_LIST[opt_idx]}", log_path, feedback)

    X, y, final_weights, coords = extract_samples(stack_path, points_layer, depth_fld, weight_fld, col_mode)
    if len(y) < 10:
    ...
    save_training_points(actual_pts_path, coords, y, final_weights, X, stack_path, points_layer.sourceCrs())
    QgsProject.instance().addMapLayer(QgsVectorLayer(actual_pts_path, "3_Actual_Model_Input_Points", "ogr"))

    results_df, best_algo_data = run_benchmarking(X, y, final_weights, sel_idx, n_iter, out_dir, feedback, opt_idx, log_path, custom_params)
    ...
    predict_map(best_algo_data['model'], stack_path, mask_path, p_map, med_size)
