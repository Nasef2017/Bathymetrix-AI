...
def generate_features(in_f, out_f, review_dir, c, b, g, r, n, selected_indices_str, do_calc, formula, mask_f, fb):
    with rasterio.open(in_f) as s:
        ...
        for idx in selected_indices:
            if idx in calculated_feats_map:
                ...
                name = FEATURE_OPTIONS[idx].replace("[", "").replace("]", "").replace(" ", "_").replace("/", "")
                p_ind = s.profile
                p_ind.update(count=1, dtype='float32', nodata=-9999.0)
...
def run_phase01_preprocessing(algorithm, parameters, context, feedback):
    ...
    input_layer = algorithm.parameterAsRasterLayer(parameters, algorithm.INPUT_RASTER, context)
    if not input_layer:
    ...
    water_poly_layer = algorithm.parameterAsVectorLayer(parameters, algorithm.INPUT_WATER_POLY, context)
    enable_auto_mask = algorithm.parameterAsBool(parameters, algorithm.ENABLE_MASKING, context)
    ...
    if water_poly_layer is not None:
        ...
        masking_choice = algorithm.parameterAsInt(parameters, algorithm.MASKING_METHOD, context)
        k_size = algorithm.parameterAsInt(parameters, algorithm.MASK_KERNEL_SIZE, context)
        if MASK_METHODS[masking_choice] == 'Otsu (Automatic NDWI)':
            adj = algorithm.parameterAsDouble(parameters, algorithm.OTSU_ADJUSTMENT, context)
            feedback.pushInfo("   [1/3] Generating Water Mask (Otsu NDWI)...")
            run_otsu_robust(curr_img, p_mask, g_idx, n_idx, adj, k_size, feedback)
        else:
            manual_val = algorithm.parameterAsDouble(parameters, algorithm.MANUAL_THRESHOLD, context)
            feedback.pushInfo("   [1/3] Generating Water Mask (Manual NDWI)...")
            run_manual_mask(curr_img, p_mask, g_idx, n_idx, manual_val, k_size, feedback)
        mask_generated = True
        final_mask_path = p_mask
    ...
    if algorithm.parameterAsBool(parameters, algorithm.APPLY_SUNGLINT, context):
        nir_g = algorithm.parameterAsInt(parameters, algorithm.NIR_BAND_SUNGLINT, context)
        perc = algorithm.parameterAsDouble(parameters, algorithm.SUNGLINT_PERCENTILE, context)
        ...
        run_hedley(curr_img, p_glint, nir_g, target_bands_idx, final_mask_path, perc, feedback)
        curr_img = p_glint
    ...
    selected_feats = algorithm.parameterAsEnums(parameters, algorithm.FEATURE_SELECTION, context)
    do_calc = algorithm.parameterAsBool(parameters, algorithm.ENABLE_BAND_CALC, context)
    calc_formula = algorithm.parameterAsString(parameters, algorithm.BAND_MATH_FORMULA, context)
    generate_features(
        curr_img, p_stack, review_dir, c_idx, b_idx, g_idx, r_idx, n_idx,
        selected_feats, do_calc, calc_formula, final_mask_path, feedback
    )
