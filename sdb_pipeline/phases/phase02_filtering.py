...
def save_subset_with_uncert(original_layer, all_features, mask, uncert, out_path, layer_name):
...
def run_phase02_filtering(algorithm, parameters, context, feedback):
    ...
    user_val = algorithm.parameterAsDouble(parameters, algorithm.RESIDUAL_THRESHOLD, context)
    max_trials = algorithm.parameterAsInt(parameters, algorithm.RANSAC_MAX_TRIALS, context)
    ...
    save_subset_with_uncert(points_layer, features_list, final_mask, est_sigma, clean_shp, "Cleaned Data")
    save_subset_with_uncert(points_layer, features_list, ~final_mask, est_sigma, reject_shp, "Rejected Data")

    plot_mult = user_val if user_val > 0 else (2.5 if mode_idx == 0 else 3.0)
    plot_physics_trend(X, y, final_mask, mode_idx, os.path.join(out_dir, '2_Plot_1_Trend.png'))
    plot_variance_analysis(y, residuals, sigma_model_for_plot, mode_idx, os.path.join(out_dir, '2_Plot_2_Variance.png'))
    plot_envelope_analysis(y, residuals, est_sigma, final_mask, plot_mult, mode_idx, os.path.join(out_dir, '2_Plot_3_Envelope.png'))
