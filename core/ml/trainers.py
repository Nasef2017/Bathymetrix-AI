...
def get_model_and_params(index, opt_idx=0):
    ...
    if index == 10:
        return (
            "SVR",
            SVR(cache_size=1000, max_iter=20000),
            (
                {"C": Real(1.0, 100.0)}
                if is_bayes
            ...
            if params and n_iter > 0:
                ...
                if current_opt_idx == 0:
                    search = RandomizedSearchCV(
                        base_model, params, n_iter=n_iter, cv=3, n_jobs=1
                    )
                elif current_opt_idx == 1:
                    search = GridSearchCV(base_model, params, cv=3, n_jobs=1)
                elif current_opt_idx == 2:
                    search = (
                        BayesSearchCV(base_model, params, n_iter=n_iter, cv=3, n_jobs=1)
                        if SKOPT_AVAILABLE
...
def run_phase03_initial_modeling(algorithm, parameters, context, feedback):
    ...
    mask_path = algorithm.parameterAsRasterLayer(
